...
parser.add_argument('--num_hard_neg', type=int, help='Create a qrel file (if it doesn\'t already exist) for the number of negatives specified')
...
weak_label_path = args.weak_label_path # /llama3_old_prompt
hard_neg_qrel_path = make_hard_neg_tsv(data_path+weak_label_path, args.num_hard_neg) 


#### Provide the data_path for corpus, queries, and qrel 
qrel_name = f"train_weak_{args.num_hard_neg}"
corpus, queries, qrels = GenericDataLoader(data_folder=data_path, qrels_folder=data_path+weak_label_path).load(split=qrel_name)
...
model_save_path = os.path.join(pathlib.Path(__file__).parent.absolute(), "output", f"dpr_weak_{len(qrels)}positive_{args.num_hard_neg}hardneg_{dataset}_{time_string}")
