...
class SentenceTransformer(nn.Sequential):
    ...
    def sample_from_topk_collate(self, batch):
        """
        Transforms a batch from a SmartBatchingDataset to a batch of tensors for the model
        Here, batch is a list of tuples: [(query, [doc, ...]), ...]

        :param batch:
            a batch from a SmartBatchingDataset
        :return:
            [[q_token, ...], [d_token, ...]], labels
        """
        texts = [[], []]
        labels = []
        for q_ds in batch: 
            query_text = q_ds[0]
            doc_text = random.choice(q_ds[1]) 
            texts[0].append(query_text)
            texts[1].append(doc_text)
            labels.append(1)            
    ...
    def fit(self,
        ...
        for dataloader in dataloaders:
            print("******************")
            print("Using Default Collate")
            dataloader.collate_fn = self.smart_batching_collate
            
            # print("Using Sample_From_TopK_Collate")
            # dataloader.collate_fn = self.sample_from_topk_collate
            print("******************")
